        ...
        func: (d, setWeatherData) => {            
            setWeatherData(prev => {
                ...
                for (let i = 0; i < d.length; i++){
                    ...
                    prev = prev.concat(...[
                        <div className="grid grid-cols-4 mt-0 mb-3" key={i}>
                            <p>{dateTime[0]}</p>
                            <p>{dateTime[1]}</p>
                            <p>{d[i].IconPhrase}</p>
        ...
        func: (d, setWeatherData) => {
            setWeatherData(prev => {
                ...
                for (let i = 0; i < forecast.length; i++){
                    ...
                    prev = prev.concat(...[
                        <div className="grid grid-cols-5 mb-3" key={`date-${i+1}`}>
                            <p>{dateTime[0]}</p>
                            <p>{dateTime[1]}</p>
                            <p>{forecast[i].Day.IconPhrase}</p>
                            <p>{forecast[i].Night.IconPhrase}</p>
                            <p>{forecast[i].Temperature.Minimum.Value}&#176;C - {forecast[i].Temperature.Maximum.Value}&#176;C</p>
                        </div>
                    ]);
                }
