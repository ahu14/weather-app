...
let Home = () => {
    ...
    let clicked = async (cityData, code) => {
        return await fetch(`http://dataservice.accuweather.com/forecasts/v1/daily/5day/${code}?apikey=${process.env.REACT_APP_WEBSITE_API_KEY}&metric=true`)
        .then(res => res.json())
        .then(d => {
            setData(prev => prev = [
                <h2 key="title" className="mt-5 font-bold">{cityData.city}, {cityData.province}, {cityData.country}</h2>
            ])
    ...
    let submitted = async (event) => {
        ...
        return await fetch(`http://dataservice.accuweather.com/locations/v1/cities/search?apikey=${process.env.REACT_APP_WEBSITE_API_KEY}&q=${city}&offset=100`)
        .then(res => res.json())
        .then(d => {
            if (d.length == 0){
